            ...
            {status === 'success' ? (
              ...
              >
                ...
                <p className="mt-2 text-[15.5px] leading-relaxed text-slatey">
                  We&apos;ll text you at {form.phone} within 24 hours with a flat-rate quote. Keep an eye on{' '}
                  {form.email} too — we&apos;ll send the certificate of insurance there.
                </p>
              </motion.div>
            ) : (
              <motion.form
              ...
              >
                <Field id="qf-name" label="Your name" value={form.name} onChange={update('name')} error={errors.name} autoComplete="name" required />
                <Field id="qf-phone" label="Phone" type="tel" inputMode="tel" value={form.phone} onChange={update('phone')} error={errors.phone} autoComplete="tel" placeholder="(913) 555-0123" required />
                <Field id="qf-address" label="Property address" value={form.address} onChange={update('address')} error={errors.address} autoComplete="street-address" className="sm:col-span-2" placeholder="123 Main St, Overland Park, KS" required />
                <Field id="qf-email" label="Email" type="email" inputMode="email" value={form.email} onChange={update('email')} error={errors.email} autoComplete="email" className="sm:col-span-2" required />
                ...
                <fieldset className="sm:col-span-2">
                  ...
                  {errors.services && <p className="mt-2 text-[13px] text-red-600">{errors.services}</p>}
                </fieldset>

                <div className="sm:col-span-2">
                  <label htmlFor="qf-notes" className="block font-display font-semibold text-[14.5px] text-ink mb-2">
                ...
                <div className="sm:col-span-2 mt-2">
                  <button type="submit" className="btn-cta w-full sm:w-auto" disabled={status === 'submitting'}>
                    {status === 'submitting' ? (
                      <>
                  ...
                  {status === 'error' && (
                    <p className="mt-2 text-[13.5px] text-red-600" role="alert">
                      ...
                      <a href="tel:+19137013077" className="underline">(913) 701-3077</a>.
                    </p>
                  )}
                </div>
              </motion.form>
              ...
              {NEXT_STEPS.map((step, i) => {
                ...
                return (
                  <li key={step.title} className="flex gap-4">
                    <span aria-hidden="true" className="grid h-10 w-10 shrink-0 place-items-center rounded-full bg-bg/10 text-cta">
...
function Field({ id, label, error, className = '', required, ...rest }) {
  return (
    <div className={className}>
      ...
      />
      {error && (
        <p id={`${id}-err`} className="mt-1 text-[13px] text-red-600">{error}</p>
      )}
    </div>
  );
}
