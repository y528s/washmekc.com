// Draggable before/after slider built with plain pointer events. No
// library. Mouse + touch + keyboard arrows + Home/End all supported.

import React, { useCallback, useEffect, useRef, useState } from 'react';
import { ClipboardCheck, Handshake, Sparkles, ShieldCheck } from 'lucide-react';
...
function Slider() {
  ...
  return (
    ...
    >
      {/* AFTER — full background. TODO: replace with /public/slider-after.jpg */}
      <div
        className="absolute inset-0"
        style={{ background: 'linear-gradient(135deg, #EEE6CD 0%, #DCC9A2 50%, #C8B281 100%)' }}
        aria-hidden="true"
      />
      {/* BEFORE — clipped to the left of divider. TODO: replace with /public/slider-before.jpg */}
      <div
        className="absolute inset-0"
        style={{
          background: 'linear-gradient(135deg, #6F7A6A 0%, #5B6452 45%, #4B5343 70%, #3F463A 100%)',
      ...
      <div className="absolute top-0 bottom-0 pointer-events-none" style={{ left: `calc(${pos}% - 1px)` }} aria-hidden="true">
      ...
      >
        <svg width="22" height="22" viewBox="0 0 22 22" fill="none" aria-hidden="true">
          <path d="M8 5 3 11l5 6M14 5l5 6-5 6" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
        </svg>
      </button>
    </div>
  );
      ...
      <ol className="mt-12 sm:mt-16 grid grid-cols-1 sm:grid-cols-4 gap-3 sm:gap-0 rounded-2xl bg-white ring-1 ring-black/5 p-4 sm:p-2 shadow-sm">
        {STEPS.map((s, i) => {
          ...
          return (
            ...
            >
              <span className="inline-flex h-10 w-10 items-center justify-center rounded-full bg-brand/10 text-brand shrink-0" aria-hidden="true">
              ...
              {i < STEPS.length - 1 && (
                <span aria-hidden="true" className="hidden sm:block absolute top-7 right-0 h-px w-6 bg-black/10" />
              )}
            </li>
          );
        })}
